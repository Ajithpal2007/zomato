...
export const RestaurantProfile = ({ restaurant, isSeller, onUpdate }: props) => {
  ...
  const saveChanges = async () => {
   ...
   const { setIsAuth, setUser } = useAppContext()
    const logoutHandler = async ()=>{
     await axios.put(`${restaurantService}/api/restaurant/status`, { status: false }, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("token")}`,
        },
      }
    ...
    <div className="mx-auto max-w-xl rounded-xl bg-white shadow-sm overflow-hidden">
      ...
      <div className="p-5 space-y-4 " >
           ...
           <div className="flex items-center justify-between">
            ...
            {isSeller && (<button title="edit" onClick={() => setEditMode(!editMode)} className="text-gray-500 hover:text-black">
              <BiEdit size={18} />
            </button>)}
        ...
        {
          editMode ? (<textarea title="description" value={description} onChange={e => setDescription(e.target.value)} className="w-full rounded border px-3 py-3 text-sm  " />)
        ...
        <div className="flex items-center justify-center pt-3 border-t">
          ...
          <div className="flex gap-3">{
            ...
            {
              isSeller && (
                <button onClick={toggleOpenSatus} className={`rounded-lg px-4 py-1.5 text-sm font-medium text-white ${isOpen ? 
               ...
               {
              isSeller && (
                <button onClick={logoutHandler} className={`rounded-lg px-4 py-1.5 text-sm font-medium text-white bg-red-600 hover:bg-red-700`} >
